...
CSV 主要列：`session_id`、`external_participant_id`、`seq`、`speaker`、`text`、`easyturn_label`、`category`、`categories`、`description`、`confidence` 等。`categories` 用 `|` 分隔最多两个原因。
...
选择中包含「生理性停顿」时，无需填写原因与心理过程或置信度。其他选择仍需完成描述和置信度。
...
- 每个标记先点选 1–2 个原因，再填写描述并点置信度
- 如果包含「生理性停顿」，描述和置信度会自动省略
- 等右上角「已保存」再提交  
